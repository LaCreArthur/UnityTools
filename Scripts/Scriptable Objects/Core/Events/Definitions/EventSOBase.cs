...
namespace Toolbox.ScriptableObjects.Events
{
    ...
    //where T : UltEventBase
    {
        ...
        [TitleGroup("Listeners"), SerializeField, InlineProperty, HideReferenceObjectPicker, ListDrawerSettings(IsReadOnly = true, Expanded = true), OnInspectorGUI("RemoveNullElements")]  
        protected List<ReferencedUltEvent<T>> ultEventListeners = new List<ReferencedUltEvent<T>>();

        public void AddListener(ReferencedUnityEvent<T> unityEvent, ReferencedUltEvent<T> ultEvent)
        {
            if (unityEventListeners == null) unityEventListeners = new List<ReferencedUnityEvent<T>>();
            unityEventListeners.Add(unityEvent);
            if (ultEventListeners == null) ultEventListeners = new List<ReferencedUltEvent<T>>();
            ultEventListeners.Add(ultEvent);
        }

        public void RemoveListener(ReferencedUnityEvent<T> unityEvent, ReferencedUltEvent<T> ultEvent)
        {
