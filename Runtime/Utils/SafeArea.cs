...
namespace AS.Toolbox.Utils
{
    ...
    public class SafeArea : MonoBehaviour
    {
        ...
        /// </summary>
        public enum SimDevice { None, iPhoneX, iPhoneXsMax, Pixel3XL_LSL, Pixel3XL_LSR }
        ...
        SimDevice GetSim()
        {
            for (int i = 0; i < DeviceSize.Length; i++)
            {
                ...
                if ((Screen.height == DeviceSize[i].x || Screen.width == DeviceSize[i].x)
                    && (Screen.height == DeviceSize[i].y || Screen.width == DeviceSize[i].y))
                {
                    return (SimDevice)i;
                }
            }
